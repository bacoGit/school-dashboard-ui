...
import { role, leaveRequestsData } from "@/lib/data";
    ...
    <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-schoolPurpleLight">
      ...
      <td>
        <div className="flex items-center gap-2">
          ...
          {role === "admin" && (
            <>
            <FormModal table="incident" type="update" id={item.id}/>
            <FormModal table="incident" type="delete" id={item.id}/>
            </>
          )}
        </div>
      </td>
  ...
  return (
    <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">All Leave Requests</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {role === "admin" && (
            <FormModal table="leaveRequest" type="create"/>
            )}
          </div>
        </div>
      </div>
      {/* List */}
      <Table columns={columns} renderRow={renderRow} data={leaveRequestsData}/>
      {/* pagination */}
      <Pagination />
    </div>
  );
